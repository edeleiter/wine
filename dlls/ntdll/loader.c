...
static void load_arm64ec_module(void)
{
    ...
    /* proton-mac: load the amd64 CPU-emulator (the xtajit64 stub now; libarm64ecfex under FEX later)
     * by BARE NAME rather than the explicit C:\windows\system32\ path. On an installed prefix the
     * bare name still resolves to system32 (find_dll_file); but in our build-tree bootstrap the
     * prefix has no files in system32 and builtins load via find_builtin_without_file, which bails
     * on any name containing a path (loader.c:3141) — so the full path can never resolve there.
     * Bare name works in both; is_prefix_bootstrap isn't reliably set at this point in loader_init. */
    load_name = module + wcslen( L"C:\\windows\\system32\\" );

    if ((status = load_dll( NULL, load_name, 0, &wm, FALSE )) ||
        (status = arm64ec_process_init( wm->ldr.DllBase )))
    {
        ERR( "could not load %s, status %lx\n", debugstr_w(load_name), status );
        NtTerminateProcess( GetCurrentProcess(), status );
    }
}
