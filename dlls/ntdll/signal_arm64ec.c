 ...
 */
/* proton-mac: guards the x18 reload's +0x98 (see [sp,#0xd68] = 0xcd0 + 0x98 below). */
C_ASSERT( offsetof( ARM64_NT_CONTEXT, X[18] ) == 0x98 );
void __attribute__((naked)) KiUserExceptionDispatcher( EXCEPTION_RECORD *rec, CONTEXT *context )
{
    asm( ".seh_proc \"#KiUserExceptionDispatcher\"\n\t"
         ...
         ".seh_endprologue\n\t"
         /* proton-mac: macOS zeroes x18 (TEB reg) on sigreturn; the arm64ec ntdll REDIRECTS
          * this dispatcher, so the signal_arm64.c reload never runs. Reload x18 from the
          * provided native CONTEXT, which .seh_context laid at sp+0xcd0 (the frame alloc),
          * so X18 is at 0xcd0 + offsetof(CONTEXT,X[18])(=0x98) = 0xd68. No-op elsewhere.
          * The X[18]==0x98 half is guarded by the C_ASSERT above KiUserExceptionDispatcher;
          * the 0xcd0 half is Wine's own .seh_stackalloc constant (kept in sync by the tie). */
         "ldr x18, [sp, #0xd68]\n\t"
         "add x0, sp, #0xcd0\n\t"
         "add x0, x0, #0x3b0\n\t"       /* rec */
         "mov x1, sp\n\t"               /* context */
