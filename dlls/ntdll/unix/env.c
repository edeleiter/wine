...
#ifdef __APPLE__
    /* proton-mac: the M4 address-space rebase forces the main image off its preferred ImageBase
     * (e.g. 0x140000000), so the server returns the informational STATUS_IMAGE_NOT_AT_BASE. The
     * image loaded fine (relocations are applied client-side); it is NOT a failure. Clear it so the
     * `if (status)` reroute below does not spuriously bounce a healthy guest through start.exe (which
     * would then need a process spawn, impossible under WINELOADERNOEXEC). */
