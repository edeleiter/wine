...
/* proton-mac: native arm64 macOS forbids mapping the low 4GB (kernel dead zone).
 * The general allocation range starts one 64KB granule above the KUSER slot
 * (0x100000000000), which carves KUSER out (the allocator never searches below
 * address_space_start). See user_shared_data below and the coherent map. */
static void *address_space_start = (void *)0x100000010000;
#else
static void *address_space_start = (void *)0x10000;
#endif
#ifdef _WIN64
#ifdef __APPLE__
/* proton-mac: native arm64 macOS refuses mappings above ~0x7f0000000000 (verified),
 * below Wine's default 0x7fffffff0000. Cap limits to the real mappable window. M4 exp. */
static void *address_space_limit = (void *)0x7f0000000000;
static void *user_space_limit    = (void *)0x7f0000000000;
static void *working_set_limit   = (void *)0x7f0000000000;
#else
...
static void mmap_init( const struct preload_info *preload_info )
{
     ...
     * proton-mac: these ranges are all in the arm64 macOS dead zone (<4GB) or above
     * the 0x7f0000000000 mmap ceiling, so reserving them is a no-op; skip. */
...
void virtual_init(void)
{
...
#ifdef __APPLE__
    /* proton-mac: the mmap-based probe over-reports on macOS (MAP_FIXED_NOREPLACE is
     * unavailable, so it can't detect that MAP_FIXED high fails). Clamp to the real
     * mappable ceiling. M4 experiment. */
    if (host_addr_space_limit > address_space_limit) host_addr_space_limit = address_space_limit;
#endif
    ...
    /* proton-mac: on macOS address_space_start is ~16TB, so `size` here is ~16TB. This is
     * intentionally DEAD on macOS: mmap_is_in_reserved_area() returns !=1 (we reserve no low
     * memory on macOS), so the guarded anon_mmap_fixed never runs. If any low reservation is
     * ever added on macOS, this would attempt a ~16TB fixed mmap of the dead zone — cap it. */
...
#ifdef __APPLE__
                             0,  /* proton-mac: low 2GB is unmappable on arm64 macOS; place the TEB block high */
#else
...
void virtual_set_large_address_space(void)
{
    if (is_win64)
    {
        if (!is_wow64())
        {
#ifndef __APPLE__  /* proton-mac: keep the high address_space_start; low 4GB is unmappable */
