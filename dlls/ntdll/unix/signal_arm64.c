...
/* proton-mac: the __wine_syscall_dispatcher_return epilogue reloads x18 from the frame
 * with a hardcoded "ldr x18, [sp, #0x90]"; guard that offset against struct changes. */
C_ASSERT( offsetof( struct syscall_frame, x[18] ) == 0x90 );
...
static void restore_context( const CONTEXT *context, ucontext_t *sigcontext )
{
    ...
    /* proton-mac: this is an ASYNC resume (suspend/resume, NtContinue, NtSetContextThread)
     * that can land mid-EC-thunk where x17 is live, so we deliberately do NOT redirect
     * through x18_resume_trampoline (which would clobber x17). macOS zeroes x18 on the
     * sigreturn; the resumed code's first [x18,#off] TEB access faults into the dead zone
     * and the reactive segv net restores x18 at that synchronous, x17-dead point. */
}
...
#ifdef __APPLE__
    /* macOS zeroes x18 (its reserved platform register) on sigreturn, so setting
     * REGn_sig(18) below is not honored. Carry the TEB in the dispatched context:
     * save_context copied the kernel-zeroed x18 into stack->context.X18, so override
     * it here; KiUserExceptionDispatcher reloads x18 from it. */
    stack->context.X18 = (ULONG_PTR)NtCurrentTeb();
#endif
 ...
 */
#ifdef __APPLE__
/* macOS zeroes x18 (its reserved platform register) on signal delivery, but Wine
 * uses x18 as the TEB register (NtCurrentTeb() reads x18). Restore it from the
 * pthread key at handler entry, before any TEB-relative code (incl. debug tracing)
 * runs — otherwise NtCurrentTeb() returns NULL and everything recurses/faults. */
static inline void restore_teb_register(void)
{
    /* "memory" clobber: x18 is the TEB base; downstream TEB reads must not be reordered
     * before this establishes it (they are volatile-asm/calls today, but harden anyway). */
...
#ifdef __APPLE__
    /* proton-mac: macOS zeroes x18 (the Windows ARM64 TEB register = Apple's reserved platform
     * register) on kernel round-trips the code doesn't control — proven for both sigreturn AND
     * the first execution of a cold mmap'd RX page (instruction-abort page-in). All Wine PE code
     * is mmap'd RX, so the first instruction of a freshly-entered code page runs with x18=0; any
     * [x18,#off] TEB access then takes a data abort at a tiny address.
     *
     * We CANNOT simply restore x18=TEB and re-run the faulting instruction: the faulting insn is a
     * data access that never retires (x18=0 -> re-faults), so the page's EXEC side never warms, and
     * any resume that branches back to it re-enters a cold page -> macOS re-zeroes x18 -> the same
...
__ASM_GLOBAL_FUNC( __wine_syscall_dispatcher_return,
                   /* proton-mac: macOS zeroes x18 (TEB reg) on sigreturn. This global entry is
                    * reached only via handle_syscall_fault's redirect, where sp == the syscall_frame,
                    * so reload x18 from frame->x[18] (0x90) before the first TEB deref. Harmless
                    * no-op elsewhere (sp==frame there too). Matches the KiUserExceptionDispatcher fix. */
                   "ldr x18, [sp, #0x90]\n\t"
                   "ldr w11, [x18, #0x380]\n\t" /* thread_data->syscall_trace */
