...
/* proton-mac: guard the hardcoded x18-reload offset below (ldr x18,[sp,#0x98]) — the
 * .seh_context lays a CONTEXT at sp, and CONTEXT.X[18] must be at 0x98 (X[]@0x008 + 18*8). */
C_ASSERT( offsetof( CONTEXT, X[18] ) == 0x98 );
...
__ASM_GLOBAL_FUNC( KiUserExceptionDispatcher,
                   ".seh_context\n\t"
                   ".seh_endprologue\n\t"
                   /* proton-mac: macOS zeroes x18 (TEB reg) on sigreturn; reload it from
                    * the dispatched context (at sp, X18 @ 0x98). Harmless no-op elsewhere. */
                   "ldr x18, [sp, #0x98]\n\t"
